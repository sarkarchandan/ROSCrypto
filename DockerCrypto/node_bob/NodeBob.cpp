#include "ros/ros.h"
#include "node_bob/cipher.h"
#include "node_bob/Crypto.hpp"
#include "node_bob/MessageArchive.hpp"
#include "node_bob/key_generator.h"
...
class CryptoPubSubHandler
{
  ...
  void MessageReceived(const node_bob::cipher::ConstPtr& _message)
  {
    ProcessMessage(_message);
    node_bob::cipher message_for_bob;
    message_for_bob.cipher = PrepareMessage();
    m_Publisher.publish(message_for_bob);
  }

  private:
  void ProcessMessage(const node_bob::cipher::ConstPtr& _message)
  {
...
int main(int argc, char **argv)
{
  ...
  ros::ServiceClient client = nodeHandle.serviceClient<node_bob::key_generator>("generate_key");
  node_bob::key_generator generator;
  ...
  if(client.call(generator))
  {
    ...
    ros::Publisher publisher = nodeHandle.advertise<node_bob::cipher>("message_for_alice",10,true);
